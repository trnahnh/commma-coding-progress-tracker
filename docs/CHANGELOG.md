...
- **Extension, CLI, Shared** — The loopback OAuth landing page — the browser tab
  shown after you authorize the editor/CLI sign-in — is now a branded, animated
  commma page instead of a bare black-and-white "connected" line. It renders the
  serif wordmark with the accent period, a pulsing status dot (green on success,
  accent on failure), the warm-paper palette with the ambient glow and noise
  texture, and staggered rise-in motion; `prefers-reduced-motion` is respected.
  The markup is generated by a new shared `authPage({ ok })` helper
  (`@commma/shared/authPage`, a dependency-free subpath export so the extension
  bundle stays lean) consumed by both `apps/extension` and `apps/cli`, replacing
  the two hand-rolled HTML strings.
